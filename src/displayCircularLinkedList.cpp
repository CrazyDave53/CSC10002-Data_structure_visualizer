...
displayCircularLinkedList::displayCircularLinkedList(sf::RenderWindow &window):
        menu(window),
        ...
        backToMenu()
{
    ...
    createMenu.mainButton->setCallback([this]{
        ...
        createRandom->setCallback([this]{
            ...
            goCreate = createFrame->addButton("GO", [this]{
                if(!inputCreate->getText().toAnsiString().empty()){
                    try{
                        circularLinkedList.createRandom(std::stoi(inputCreate->getText().toAnsiString()));
                    }
        ...
        createFromFile->setCallback([this]{
            ...
            goCreate = createFrame->addButton("      Browse      ", [this]{
                circularLinkedList.createFromFile();
            });
        ...
        createUserInput->setCallback([this]{
            ...
            goCreate = createFrame->addButton("GO", [this]{
                if(!inputCreate->getText().toAnsiString().empty())
                    circularLinkedList.createUserInput(inputCreate->getText().toAnsiString());
            });
        });
    ...
    searchMenu.mainButton->setCallback([this]{
        ...
        goSearch = searchMenu.frame->addButton("GO",[this]{
            if(!inputSearchValue->getText().toAnsiString().empty()){
                try{
                    circularLinkedList.search(std::stoi(inputSearchValue->getText().toAnsiString()));
    ...
    updateMenu.mainButton->setCallback([this]{
        ...
        goUpdate = updateMenu.frame->addButton("GO", [this]{
            if(!inputUpdateIndex->getText().toAnsiString().empty() && !inputUpdateValue->getText().toAnsiString().empty()){
                try{
                    circularLinkedList.updateNode(std::stoi(inputUpdateIndex->getText().toAnsiString()), std::stoi(inputUpdateValue->getText().toAnsiString()));
                }catch(std::invalid_argument){
                    //do nothing
                }
            }
    ...
    insertMenu.mainButton->setCallback([this]{
        ...
        insertHead->setCallback([this]{
            ...
            goInsert = insertFrame->addButton("GO", [this]{
                if(!inputInsertValue->getText().toAnsiString().empty())
                    try{
                        circularLinkedList.insertToHead(std::stoi(inputInsertValue->getText().toAnsiString()));
                    }catch(std::invalid_argument){
                        //do nothing
                    }
            });
        ...
        insertTail->setCallback([this]{
            ...
            goInsert = insertFrame->addButton("GO", [this]{
                if(!inputInsertValue->getText().toAnsiString().empty()){
                    try{
                        circularLinkedList.insertToTail(std::stoi(inputInsertValue->getText().toAnsiString()));
                    }catch(std::invalid_argument){
                        //do nothing
                    }
                }
        ...
        insertArbitrary->setCallback([this]{
            ...
            goInsert = insertFrame->addButton("GO", [this]{
                if(!inputInsertValue->getText().toAnsiString().empty() && !inputInsertIndex->getText().toAnsiString().empty()) {
                    try {
                        circularLinkedList.insertArbitrary(std::stoi(inputInsertValue->getText().toAnsiString()),
        ...
        deleteArbitrary->setCallback([this]{
            ...
            goDelete = deleteFrame->addButton("GO", [this]{
                if(!inputDeleteIndex->getText().toAnsiString().empty()){
                    try{
                        circularLinkedList.deleteArbitrary(std::stoi(inputDeleteIndex->getText().toAnsiString()));
                    }catch(std::invalid_argument){
                        //do nothing
                    }
                }
    ...
    mediaBar.m_forwardButton->setCallback([this]{
        circularLinkedList.fastForward();
    });
    mediaBar.m_backwardButton->setCallback([this]{
        circularLinkedList.rewind();
    });
    mediaBar.m_playButton->setCallback([this]{
        if(circularLinkedList.isPausing)
...
displayCircularLinkedList::~displayCircularLinkedList() {

}

windowType displayCircularLinkedList::mainloop(sf::RenderWindow &window) {
    nextWindow = circularLinkedListWindow;
    while (window.isOpen() && nextWindow == circularLinkedListWindow)
    {
        sf::Event event;
        ...
        while (window.pollEvent(event))
        {
            // Send events to menu
            menu.onEvent(event);
            circularLinkedList.onEvent(event);
...
//            menu.recomputeGeometry();
        ...
        mediaBar.setPauseState(circularLinkedList.isPausing);
