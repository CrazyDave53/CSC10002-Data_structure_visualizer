...
displayDoublyLinkedList::displayDoublyLinkedList(sf::RenderWindow& window) :
    menu(window),
    ...
    createMenu.mainButton->setCallback([this]{
        ...
        createRandom->setCallback([this]{
            ...
            goCreate = createFrame->addButton("GO", [this]{
                if(!inputCreate->getText().toAnsiString().empty()){
                    try{
                        doublyLinkedList.createRandom(std::stoi(inputCreate->getText().toAnsiString()));
                    }
        ...
        createFromFile->setCallback([this]{
            ...
            goCreate = createFrame->addButton("      Browse      ", [this]{
                doublyLinkedList.createFromFile();
            });
        ...
        createUserInput->setCallback([this]{
            ...
            goCreate = createFrame->addButton("GO", [this]{
                if(!inputCreate->getText().toAnsiString().empty())
                    doublyLinkedList.createUserInput(inputCreate->getText().toAnsiString());
            });
        });
    ...
    searchMenu.mainButton->setCallback([this]{
        ...
        goSearch = searchMenu.frame->addButton("GO",[this]{
            if(!inputSearchValue->getText().toAnsiString().empty()){
                try{
                    doublyLinkedList.search(std::stoi(inputSearchValue->getText().toAnsiString()));
    ...
    updateMenu.mainButton->setCallback([this]{
        ...
        goUpdate = updateMenu.frame->addButton("GO", [this]{
            if(!inputUpdateIndex->getText().toAnsiString().empty() && !inputUpdateValue->getText().toAnsiString().empty()){
                try{
                    doublyLinkedList.updateNode(std::stoi(inputUpdateIndex->getText().toAnsiString()), std::stoi(inputUpdateValue->getText().toAnsiString()));
                }catch(std::invalid_argument){
                    //do nothing
                }
            }
    ...
    insertMenu.mainButton->setCallback([this]{
        ...
        insertHead->setCallback([this]{
            ...
            goInsert = insertFrame->addButton("GO", [this]{
                if(!inputInsertValue->getText().toAnsiString().empty())
                    try{
                        doublyLinkedList.insertToHead(std::stoi(inputInsertValue->getText().toAnsiString()));
                    }catch(std::invalid_argument){
                        //do nothing
                    }
            });
        ...
        insertTail->setCallback([this]{
            ...
            goInsert = insertFrame->addButton("GO", [this]{
                if(!inputInsertValue->getText().toAnsiString().empty()){
                    try{
                        doublyLinkedList.insertToTail(std::stoi(inputInsertValue->getText().toAnsiString()));
                    }catch(std::invalid_argument){
                        //do nothing
                    }
                }
        ...
        insertArbitrary->setCallback([this]{
            ...
            goInsert = insertFrame->addButton("GO", [this]{
                if(!inputInsertValue->getText().toAnsiString().empty() && !inputInsertIndex->getText().toAnsiString().empty()) {
                    try {
                        doublyLinkedList.insertArbitrary(std::stoi(inputInsertValue->getText().toAnsiString()),
        ...
        deleteArbitrary->setCallback([this]{
            ...
            goDelete = deleteFrame->addButton("GO", [this]{
                if(!inputDeleteIndex->getText().toAnsiString().empty()){
                    try{
                        doublyLinkedList.deleteArbitrary(std::stoi(inputDeleteIndex->getText().toAnsiString()));
                    }catch(std::invalid_argument){
                        //do nothing
                    }
                }
...
displayDoublyLinkedList::~displayDoublyLinkedList() {

}

windowType displayDoublyLinkedList::mainloop(sf::RenderWindow &window) {
    nextWindow = doublyLinkedListWindow;
    changeWindow->selectItem(1);
    while (window.isOpen() && nextWindow == doublyLinkedListWindow)
    {
        sf::Event event;
        ...
        while (window.pollEvent(event))
        {
            // Send events to menu
            menu.onEvent(event);
            doublyLinkedList.onEvent(event);
...
//            menu.recomputeGeometry();
        ...
        mediaBar.setPauseState(doublyLinkedList.isPausing);
